...
def main():
    ...
    testset_name = 'LIVE1_gray' # 'LIVE1_gray' 'Classic5' 
    ...
    for quality_factor in quality_factor_list:
        ...
        H_paths = [img for img in util.get_image_paths(H_path) if not os.path.basename(img).startswith('._')]
        ...
        for idx, img in enumerate(H_paths):
            ...
            if img_L is None or img_L.size == 0:
                logger.error(f"Image {img} is empty or failed to load.")
                continue

            logger.info(f"Loaded image {img} with shape: {img_L.shape}")

            _, encimg = cv2.imencode('.jpg', img_L, [int(cv2.IMWRITE_JPEG_QUALITY), quality_factor])

            if not _ or encimg is None:
                logger.error(f"JPEG encoding failed for image {img}")
                continue

            img_L = cv2.imdecode(encimg, 0) if n_channels == 1 else cv2.imdecode(encimg, 3)

            if img_L is None or img_L.size == 0:
                logger.error(f"JPEG decoding failed for image {img}")
                continue
